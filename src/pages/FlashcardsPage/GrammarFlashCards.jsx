      ...
      >
        ...
        >
          ...
          <div className="mt-12">
            ...
            {card?.usage && (
              <div className="text-base italic text-gray-600 mb-2 text-center">
                {card.usage}
              </div>
            )}
            {/* Example */}
...
export default function FlashcardsPage(words = []) {
  ...
  const getKnownWords = async () => {
    try {
      const response = await axios.get(
        `/api/progress-grammars/known?lessonId=${words?.lessonId}`
      );
      setKnown(response.data.map((item) => item.grammarId));
    } catch (error) {
  ...
  const toggleKnown = async () => {
    const id = current?.grammarId;
    const isMastered = known.includes(Number(current?.grammarId));
    try {
      await axios.patch(`/api/progress-grammars/${id}/complete`, {
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <Flashcard
                  ...
                  isKnown={known.includes(Number(current?.grammarId))}
                  playAudio={playAudio}
                />
              </motion.div>
            </AnimatePresence>
