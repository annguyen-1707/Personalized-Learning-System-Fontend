...
import { FiVolume2, FiArrowRight, FiArrowLeft, FiRefreshCw, FiPlus, FiCheck, FiDownload } from 'react-icons/fi';
...
function Flashcard({ card, flipped, setFlipped, isKnown, isSaved, playAudio }) {
  return (
    ...
    >
      ...
      >
        {/* Front */}
        <div className={`absolute w-full h-full rounded-xl shadow-lg bg-white border-2 ${isSaved ? 'border-accent-500' : 'border-gray-200'} flex flex-col items-center justify-center p-6`} style={{ backfaceVisibility: 'hidden' }}>
          <div className="text-4xl font-bold mb-2 text-primary-700">{card.frontText}</div>
          {card.romaji && <div className="text-base italic text-gray-600 mb-2">{card.romaji}</div>}
          {card.subText && <div className="text-lg text-gray-600 mb-4">{card.subText}</div>}
          <button
            onClick={e => { e.stopPropagation(); playAudio(card.frontText); }}
        ...
        <div className={`absolute w-full h-full rounded-xl shadow-lg bg-white border-2 ${isSaved ? 'border-accent-500' : 'border-gray-200'} flex flex-col items-start justify-start p-6`} style={{ backfaceVisibility: 'hidden', transform: 'rotateY(180deg)' }}>
          ...
          {card.example && (
            <div className="mt-4 text-sm w-full">
              ...
              <div className="italic text-gray-700">{card.example.japanese}</div>
...
// Controls Component
function FlashcardControls({ onPrev, onNext, onFlip, onKnown, onSave, flipped, isKnown, isSaved }) {
  return (
    <div className="flex flex-wrap justify-center gap-2 w-full max-w-md">
      <button onClick={onPrev} className="p-3 rounded-full bg-white border border-gray-300 hover:bg-gray-50" title="Previous">
        <FiArrowLeft className="h-5 w-5" />
      </button>
      <button onClick={onFlip} className="p-3 rounded-full bg-primary-500 text-white hover:bg-primary-600" title="Flip">
        <FiRefreshCw className="h-5 w-5" />
      </button>
      <button
        onClick={onKnown}
        className={`p-3 rounded-full ${isKnown ? 'bg-success-100 text-success-700 border-success-300' : 'bg-success-500 text-white'} hover:bg-success-200`}
        title={isKnown ? 'Mark Unknown' : 'Mark Known'}
      >
        <FiCheck className={`h-5 w-5 ${isKnown ? 'opacity-50' : ''}`} />
      </button>
      <button onClick={onNext} className="p-3 rounded-full bg-white border border-gray-300 hover:bg-gray-50" title="Next">
    ...
    <div className="flex justify-between items-center w-full max-w-md mb-6">
      ...
      <div className="w-16 h-16">
        <CircularProgressbar
          value={percent}
          text={`${percent}%`}
          styles={buildStyles({ textSize: '1.5rem', pathTransitionDuration: 0.5 })}
        />
      </div>
      <div className="text-center">
...
export default function FlashcardsPage() {
    ...
    if (!known.includes(cardId) && !inProgress.includes(cardId)) {
      try {
        await axios.patch('http://localhost:8080/flashcards/status', null, {
          params: { type, favoriteListId, flashcardId: cardId, status: 'IN_PROGRESS' }
        });
  ...
  useEffect(() => {
    ...
    axios
      .get('http://localhost:8080/flashcards', { params: { type, favoriteListId } })
      .then(res => {
        const data = res.data.data || [];
        setCards(data);
        setIdx(0);
        setFlipped(false);
        setKnown(data.filter(card => card.status === 'MASTERED').map(card => card.id));
        setInProgress(data.filter(card => card.status === 'IN_PROGRESS').map(card => card.id));
    ...
    <>
      <div className="max-w-4xl mx-auto p-4">
        <header className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Flashcards: {type}</h1>
        </header>
        <section className="flex flex-col items-center mb-8">
          ...
          <FlashcardStats currentIndex={idx} totalCards={cards.length} knownCards={known.length} />
          <FlashcardControls
            onPrev={prev}
            onNext={next}
